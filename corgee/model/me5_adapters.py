...
def xlmroberta_forward_self_attention_with_rope(
    ...
    query_layer, key_layer = apply_rotary_pos_emb(
        query_layer, key_layer, cos, sin, pos_ids, pos_ids
    )
...
def fuse_rope_into_me5(st):
    ...
    head_dim = (
        st[0].auto_model.config.hidden_size
        // st[0].auto_model.config.num_attention_heads
    )
    ...
    for layer in st[0].auto_model.encoder.layer:
        ...
        forward_fn = partial(
            xlmroberta_forward_self_attention_with_rope, rotary_emb=rotary_emb
        )
...
class XLMRobertaIntermediateGLU(nn.Module):
    def __init__(self, orig_layer):
        ...
        self.dense = nn.Linear(
            orig_layer.dense.in_features, orig_layer.dense.out_features, bias=True
        )
