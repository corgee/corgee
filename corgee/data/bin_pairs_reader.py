...
def fill_dense_array(data, densemat, maxlen=128):
    ...
    while offset < data.shape[0]:
        datal = data[offset]
        _l = min(datal, maxlen)
        densemat[rownum, :_l] = data[offset + 1 : offset + _l + 1]
        offset += datal + 1
        rownum += 1
...
def fill_dense_array_pairs(data, densemat1, densemat2):
    offset, numrows, maxlen1, maxlen2 = (
        0,
        densemat1.shape[0],
        densemat1.shape[1],
        densemat2.shape[1],
...
class BinLPairsDataLoader:
    ...
    def __init__(self, config, rank, world_size, device, *args, **kwargs):
        ...
        ), "batch_size needs to be a multiple of world_size"
    ...
    def _populate_file_contents_queue(self, file_path):
        ...
        if not self.stopped:
            ...
            feat1, feat2 = read_pairs(
                file_data, self.maxlen1, self.maxlen2, self.padding_index
            )
    ...
    def _submit_file_reads_async(self):
        while not self.stopped:
            ...
            _res = self._file_reader_pool.map(
                self._populate_file_contents_queue, self.all_files
            )
    ...
    def _create_batches_async(self):
        while not self.stopped:
            ...
            logger.info(
                f"creating batches from {file_path} already prepared batches {self.batches_prepared}"
            )
    ...
    def __iter__(self):
        while self.num_batches < self.num_steps:
            ...
            feats2_dest = torch.empty(
                (self.batch_size // self.world_size, self.maxlen2),
                device="cuda",
                dtype=torch.int32,
            )
            ...
            if self.cut_percentile is not None:
                ...
                percentile_pos = min(
                    int(feats1.shape[0] * self.cut_percentile), feats1.shape[0] - 1
                )
                ...
                cut_feat2 = torch.sort((feats2 != self.padding_index).sum(1)).values[
                    percentile_pos
                ]
                feats2 = feats2[
...
class BinLPairsDataLoaderMultitask:
    ...
    def __init__(self, config, rank, world_size, device, *args, **kwargs):
        ...
        num_steps_datasets = np.array(
            [data_config["files"][d]["num_steps"] for d in self.datasets],
            dtype=np.int32,
        )
        self.num_steps, self.num_batches = num_steps_datasets.sum(), 0

        self.order = np.array(
            sum([[i] * _len for i, _len in enumerate(num_steps_datasets)], [])
        )
