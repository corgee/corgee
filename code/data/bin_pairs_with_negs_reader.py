...
class BinLPairsWithNegativesDataLoader(BinLPairsDataLoader):
    ...
    def __init__(self, config, rank, world_size, device, *args, **kwargs):
        ...
        super(BinLPairsWithNegativesDataLoader, self).__init__(config, rank, world_size, device)

    def _populate_file_contents_queue(self, file_path):
        # check whether pool has been stopped before starting file read
        if not self.stopped:
            feat1, feat2 = read_pairs_with_negs(self._read_file(file_path), self.maxlen1, self.maxlen2, self.num_negatives)
        ...
        while self.num_batches < self.num_steps:
            ...
            if self.rank == 0:
                ...
                if self.prefix1 is not None:
                    ...
                    feats1 = torch.cat((self.prefix1[None, :].repeat(bsz1, 1), feats1), dim=1)[:, :numtok1]
                if self.prefix2 is not None:
                    bsz2, num_ex, numtok2 = feats2.shape
                    feats2 = torch.cat((self.prefix2[None, None, :].repeat(bsz2, num_ex, 1), feats2), dim=2)[:, :, :numtok2]
                ...
                if self.suffix2 is not None:
                    start_cols = torch.minimum((feats2>0).sum(2)+len(self.suffix2), torch.tensor(self.maxlen2))-len(self.suffix2)
                    batch_indices = torch.arange(feats2.size(0)).unsqueeze(1).expand(-1, feats2.size(1))
                    dim2_indices = torch.arange(feats2.size(1)).expand(feats2.size(0), -1)
                    for _i in range(len(self.suffix2)):
                        feats2[batch_indices, dim2_indices, start_cols-1] = self.suffix2[_i]
            ...
            feats1_dest = torch.empty((self.batch_size//self.world_size, self.maxlen1), device='cuda',
                                      dtype=torch.int32)
            feats2_dest = torch.empty((self.batch_size//self.world_size, self.num_negatives+1, self.maxlen2),
            ...
            if self.cut_percentile is not None:
                ...
                cut_feat1 = torch.sort((feats1!=self.padding_index).sum(1)).values[percentile_pos1]
                feats1 = feats1[:, :next_multiple(cut_feat1, self.cut_percentile_multiple)]
                percentile_pos2 = min(int(bsz*num_ex*self.cut_percentile), bsz*num_ex-1)
                cut_feat2 = torch.sort((feats2!=self.padding_index).sum(2).ravel()).values[percentile_pos2]
...
class BinLPairsWithNegativesDataLoaderMultitask():
    def __init__(self, config, rank, world_size, device, *args, **kwargs):
        ...
        self.sample_probs = np.array([data_config['files'][d]['rate'] for d in self.datasets], dtype=np.float32)
