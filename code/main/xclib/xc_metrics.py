...
import numpy as np
from main.xclib.sparse import topk, binarize
import warnings
...
def precision_recall(X, true_labels, k=5):
    indices, true_labels, _, _ = _setup_metric(X, true_labels, k=k)
    deno = true_labels.sum(axis=1)
    deno[deno == 0] = 1
    deno = 1/deno
    eval_flags = _eval_flags(indices, true_labels, None)
    return _precision(eval_flags, k), _recall(eval_flags, deno, k)
...
def mrr(X, true_labels, k=5):
    """
    Compute mrr@k for 1-k
    ...
    Returns:
    -------
    np.ndarray: mrr values for 1-k
    """
    mrrs_allpos = 1/(1+np.repeat(np.arange(k)[None, :], X.shape[0], axis=0))
    indices, true_labels, _, _ = _setup_metric(X, true_labels, k=k)
    eval_flags = _eval_flags(indices, true_labels, None)
    return np.cumsum(mrrs_allpos*eval_flags, axis=-1).mean(0)
