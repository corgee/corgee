...
from model.transformer.nomicbert.block import Block
from model.transformer.nomicbert.embedding import BertEmbeddings
from model.transformer.nomicbert.configuration_nomic_bert import NomicBertConfig
from model.transformer.nomicbert.bert import remap_bert_state_dict
from model.transformer.nomicbert.model_utils import filter_shapes, state_dict_from_pretrained
...
class NomicBertForPreTraining(NomicBertPreTrainedModel):
    ...
    def __init__(self, config: GPT2Config):
        ...
        # needed for muP
        self.mlm_logit_scale = config.mlm_logit_scale
        logger.info(f'mlm logit scale {self.mlm_logit_scale}')
    ...
    def forward(
        ...
        prediction_scores = self.cls(sequence_output) * self.mlm_logit_scale
