    ...
    def __init__(self, config, rank, world_size, device):
        ...
        for key, enc_conf in config['encoders'].items():
            if isinstance(enc_conf, str):
                ...
                logger.info(f'encoder {key} #params emb {emb_p:.2e} non-emb {non_emb_p:.2e} total {tot_p:.2e}')

        self.encoders = nn.ModuleDict(self.encoders)
        self.loss = None
        if 'loss' in config:
            self.loss = create_loss(config['loss'], self.encoders, rank, world_size, device)
        ...
        if world_size > 1:
            for k in self.encoders:
                if len(list(p for p in self.encoders[k].parameters() if p.requires_grad)) > 0:
                    self.encoders[k] = nn.parallel.DistributedDataParallel(self.encoders[k],
                        device_ids=[device],
            ...
            if isinstance(self.loss, nn.Module):
                if len(list(p for p in self.loss.parameters() if p.requires_grad)) > 0:
                    self.loss = nn.parallel.DistributedDataParallel(self.loss,
                        device_ids=[device],
                        broadcast_buffers=False,
    ...
    def forward(self, batch_data, return_layerwise=False):
        if return_layerwise:
            ...
            for k, v in batch_data.items():
                if v is not None:
                    res[k], layerwise = self.encoder_forward(k, v, return_layerwise=True)
                    # store the layerwise embeddings as key%%index in the dict
                    res.update({k+f'%%{i}': layer for i, layer in enumerate(layerwise)})
                else:
                    res[k] = None
            return res
        else:
            return {k: self.encoder_forward(k, v) if v is not None else None for k, v in batch_data.items()}
    ...
    def no_sync(self):
        if self.world_size > 1:
            with ExitStack() as stack:
                for k in self.encoders:
                    if isinstance(self.encoders[k], nn.parallel.DistributedDataParallel):
