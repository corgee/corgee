...
class RerankingEvaluator(Evaluator):
    ...
    def compute_metrics_batched(self, model, **kwargs):
        """
        ...
        all_docs_embs = np.asarray(encode_corpus_func(all_docs, batch_size=self.batch_size, **kwargs))
    ...
    def compute_metrics_individual(self, model, **kwargs):
        """
        ...
        for instance in tqdm.tqdm(self.samples, desc="Samples"):
            ...
            if isinstance(query, str):
                ...
                query = [query]
            query_emb = np.asarray(encode_queries_func(query, batch_size=self.batch_size, **kwargs))
            docs_emb = np.asarray(encode_corpus_func(docs, batch_size=self.batch_size, **kwargs))
