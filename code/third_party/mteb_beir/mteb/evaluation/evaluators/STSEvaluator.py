...
class STSEvaluator(Evaluator):
    ...
    def __call__(self, model, **kwargs):
        ...
        embeddings1 = np.asarray(model.encode(self.sentences1, batch_size=self.batch_size, **kwargs))
        logger.info(f"Encoding {len(self.sentences2)} sentences2...")
        embeddings2 = np.asarray(model.encode(self.sentences2, batch_size=self.batch_size, **kwargs))
