...
class kNNClassificationEvaluator(Evaluator):
    ...
    def __call__(self, model, test_cache=None, **kwargs):
        scores = {}
        max_accuracy = 0
        max_f1 = 0
        max_ap = 0
        X_train = np.asarray(model.encode(self.sentences_train, batch_size=self.batch_size, **kwargs))
        if test_cache is None:
            X_test = np.asarray(model.encode(self.sentences_test, batch_size=self.batch_size, **kwargs))
            test_cache = X_test
        else:
            X_test = test_cache
