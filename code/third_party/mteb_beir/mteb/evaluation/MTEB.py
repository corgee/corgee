        ...
        **kwargs
        ...
        while len(self.tasks) > 0:
            ...
            try:
                ...
                # # skipping actual evaluation: to test data loading and statistics computation
                # if "unload_data" in dir(task):
                #     logger.info("unloading data")
                #     task.unload_data(eval_splits=task_eval_splits)
                # del self.tasks[0]
                # continue
                ...
                for split in task_eval_splits:
                    tick = time()
                    results = task.evaluate(model, split, output_folder=output_folder,
                                            task_type=task.description['type'],
                                            task_name=task.description['name'], **kwargs)
                    tock = time()
