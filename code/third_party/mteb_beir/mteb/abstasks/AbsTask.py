...
class AbsTask(ABC):
    def __init__(self, seed=42, **kwargs):
        ...
        self.hf_repo_cache_dir_ = kwargs.get('hf_repo_cache_dir_', None)
    ...
    def load_data(self, **kwargs):
        ...
        hf_hub_name = self.description["hf_hub_name"]
        if self.hf_repo_cache_dir_ is not None:
            hf_hub_name = os.path.join(self.hf_repo_cache_dir_, hf_hub_name)
            assert os.path.isdir(hf_hub_name)
        self.dataset = datasets.load_dataset(
            hf_hub_name, revision=self.description.get("revision", None)
        )
