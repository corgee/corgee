...
class AbsTaskRetrieval(AbsTask):
    ...
    def evaluate(
        ...
        **kwargs
        ...
        # removed the distributed implementation as it only works for SentenceTransformer and was shaky 
        model = DRES(
            model,
        ...
        save_path, return_results= None, True
        if self.description['main_score'] == 'save_results':
            assert output_folder is not None
            return_results = False
            save_path = os.path.join(output_folder, f"{self.description['name']}_results")

        retriever = EvaluateRetrieval(model, score_function=score_function)  # or "cos_sim" or "dot"
        start_time = time()
        results = retriever.retrieve(corpus, queries, save_path=save_path, return_results=return_results, **kwargs)
        end_time = time()
